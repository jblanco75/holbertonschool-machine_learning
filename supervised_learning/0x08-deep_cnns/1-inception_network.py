...
def inception_network():
    ...
    conv_1 = K.layers.Conv2D(filters=64,
                             ...
                             activation='relu',
                             strides=2,
                             kernel_initializer=w)(input)
    pool_1 = K.layers.MaxPooling2D(pool_size=3, strides=2,
                                   padding='same')(conv_1)
    conv_2R = K.layers.Conv2D(filters=64,
                              kernel_size=(1, 1),
                              padding='same',
                              activation='relu',
                              strides=1,
    ...
    conv_2 = K.layers.Conv2D(filters=192,
                             ...
                             activation='relu',
                             strides=1,
                             kernel_initializer=w)(conv_2R)
    pool_2 = K.layers.MaxPooling2D(pool_size=3, strides=2,
                                   padding='same')(conv_2)
    inblock_3a = inception_block(pool_2, [64, 96, 128, 16, 32, 32])
    inblock_3b = inception_block(inblock_3a, [128, 128, 192, 32, 96, 64])
    pool_3 = K.layers.MaxPooling2D(pool_size=3, strides=2,
    ...
    pool_4 = K.layers.MaxPooling2D(pool_size=3, strides=2,
